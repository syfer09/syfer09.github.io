...
For each array size $$n$$, find the smallest possible array that satisfies these conditions. We say array $$a$$ is smaller than $$b$$ if $$a_i$$ $$a_i\leqb_i$$ in the first place where $$a$$ and $$b$$ differ.
...
Our second operation is to take some element $$x\in S$$ and replace it with $$\abs{x-k}$$.
